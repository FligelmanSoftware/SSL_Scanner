...
class Scanner
    ...
    def check_s_client(remote_server, port)
        ...
        renegotiation = "As sec".colorize(:red)
        ...
        if results =~ /Secure Renegotiation IS supported/i
            renegotiation = "As sec".colorize(:green)
        end
...
parser = OptionParser.new do |opts|
    ...
    opts.on('-c', '--certificate', 'Displays certificate info') do
        options[:check_cert] = true
    end

    opts.on('-o', '--output filename', 'File to save results') do |filename|
